...
ErrorStack push_stack(stack_t *stack_t, void *value)
{
...
static ErrorStack reallocation_stack(stack_t *stack_t)
{
    ...
    if (stack_t->size < stack_t->capacity / SIZE_REALLOC / SIZE_REALLOC)
    {
        temp = realloc(stack_t->ptr, stack_t->capacity / SIZE_REALLOC / SIZE_REALLOC);
        if (temp == NULL)
            return STACK_ERROR_ALLOCATION;
